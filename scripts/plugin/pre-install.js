#!/usr/bin/env node
/**
 * Pre-Install Script for FMI Plugin
 *
...
function main() {
  console.log("\n=== FMI COC Claude (Python) - Pre Install ===\n");
  ...
  if (nodeVersion >= MIN_NODE_VERSION) {
    console.log(
      `✓ Node.js version: ${process.versions.node} (requires >= ${MIN_NODE_VERSION})`,
    );
  ...
  try {
    ...
    if (freeGB >= 0.1) {
      ...
      console.log(
        `✗ Disk space: ${freeGB.toFixed(1)} GB (need at least 100MB)`,
      );
